...
let droneNodes    = [];
let masterGain   = null;
let audioMuted   = (localStorage.getItem('cu_mute') || '0') === '1';

let stepReady       = true;  // debounce guard for initiation steps
let isTransitioning = false; // blocks taps during screen transitions

// ─── INPUT HELPERS ───
function onTap(el, fn, opts = {}) {
  if (!el) return;
  el.addEventListener('pointerup', (e) => {
    if (opts.blockDuringTransition && isTransitioning) return;
...
function announce(msg) {
  ...
  if (!live) return;
  live.textContent = '';
  // slight delay ensures SR announces repeated strings
...
function initAudio() {
  ...
  try {
    ...
    masterGain.connect(masterGain || audioCtx.destination);
  } catch(e) {}
}
}
function tryDrone() {
...
function playDrone() {
  ...
  [432, 216, 144, 108].forEach((f, i) => {
    ...
    o.connect(g); g.connect(masterGain || audioCtx.destination); o.start();
    droneNodes.push({ o, g });
  });
}
function stopDrone() {
...
function playCollapseSound() {
  ...
  b.connect(bg); bg.connect(masterGain || audioCtx.destination);
...
function updateAudioBtn() {
  ...
  btn.setAttribute('aria-label', audioMuted ? (lang === 'en' ? 'Sound off' : 'Sonido apagado') : (lang === 'en' ? 'Sound on' : 'Sonido encendido'));
}
onTap(document.getElementById('langBtn'), () => {
  setLang(lang === 'en' ? 'es' : 'en');
  updateAudioBtn();
  const active = document.querySelector('.screen.active');
  if (active && active.id === 's-field') buildField();
  if (active && active.id === 's-init')  buildInit();
}, { blockDuringTransition: true });
onTap(document.getElementById('fontBtn'), () => {
  largeFnt = !largeFnt;
...
// ─── AUDIO TOGGLE ───
onTap(document.getElementById('audioBtn'), () => {
  audioMuted = !audioMuted;
  localStorage.setItem('cu_mute', audioMuted ? '1' : '0');
  initAudio();
  ...
  if (masterGain) {
    ...
    masterGain.gain.linearRampToValueAtTime(audioMuted ? 0 : 1, now + 0.08);
  }
  updateAudioBtn();
  if (audioMuted) stopDrone(); else tryDrone();
  announce(audioMuted ? (lang === 'en' ? 'Sound off' : 'Sonido apagado') : (lang === 'en' ? 'Sound on' : 'Sonido encendido'));
  if (navigator.vibrate) navigator.vibrate(audioMuted ? [15, 30, 15] : 22);
});
...
function buildInit() {
  ...
  steps.forEach((s, i) => {
    ...
    div.innerHTML = h;
    if (s.isLast) onTap(div.querySelector('#readyBtn'), enterField, { blockDuringTransition: true });
...
document.getElementById('s-init').addEventListener('pointerup', e => {
...
function buildField() {
  ...
  STATES[lang].forEach(st => {
    ...
    o.type = 'button';
    o.className = 'orb';
    o.setAttribute('aria-label', st.name);
    o.dataset.state = st.name;
    ...
    onTap(o, () => selectState(st), { blockDuringTransition: true });
    grid.appendChild(o);
  });
  ...
  orbs.forEach((b, i) => b.tabIndex = (i === 0 ? 0 : -1));
  grid.onkeydown = (e) => {
    const keys = ['ArrowRight','ArrowLeft','ArrowDown','ArrowUp','Home','End'];
    if (!keys.includes(e.key)) return;
    const cur = document.activeElement;
    const idx = orbs.indexOf(cur);
    let next = idx;
    if (e.key === 'Home') next = 0;
    else if (e.key === 'End') next = orbs.length - 1;
    else {
      const dir = (e.key === 'ArrowRight' || e.key === 'ArrowDown') ? 1 : -1;
      next = Math.max(0, Math.min(orbs.length - 1, (idx === -1 ? 0 : idx + dir)));
    }
    orbs.forEach((b, i) => b.tabIndex = (i === next ? 0 : -1));
    orbs[next].focus();
    e.preventDefault();
  };
...
// ─── REVISIT INITIATION ───
onTap(document.getElementById('revisitBtn'), () => {
  buildInit();
...
document.getElementById('s-collapse').addEventListener('pointerup', e => {
...
// ─── RETURN TO FIELD ───
document.getElementById('retBtn').addEventListener('pointerup', () => {
  clearAllBreath();
  collapseStage = 0;
...
// ─── STILL MODE ───
onTap(document.getElementById('gStill'), enterStill, { blockDuringTransition: true });
function enterStill() {
...
document.getElementById('stillBack').addEventListener('pointerup', () => {
  clearInterval(stillT);
  crossFade('s-still', 's-field', 1.0, () => buildField());
});

// ─── BREATH GLYPH ───
onTap(document.getElementById('gBreath'), breathGlyph, { blockDuringTransition: true });
